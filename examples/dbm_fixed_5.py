...
def make_dbm((X_train, X_val), rbms, (Q, G, H), args):
    print "\nMake dbm # ...\n\n"
    dbm = DBM(rbms=rbms,
...
parser.add_argument('--n-hiddens', type=int, default=(0), metavar='N', nargs='+',
...
parser.add_argument('--sparsity-target', type=float, default=(0.2, 0.1, 0.05), metavar='T', nargs='+',
                    help='desired probability of hidden activation')
parser.add_argument('--sparsity-cost', type=float, default=(1e-4, 5e-5, 3e-6), metavar='C', nargs='+',
...
space = {
    ...
    'units3': hp.quniform('units3', 0, 100, 20),#implementation of hq.uniform is weird see github.com/hyperopt/hyperopt/issues/321
...
def objective(params):
    ...
    # jointly train DBM
    dbm = make_dbm((X_train, X_val), (rbm1, rbm2, rbm3), (Q, G, H), args)
