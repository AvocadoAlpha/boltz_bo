...
def make_rbm_n(Q, args,i): # is for which layer to make start with 2
    ...
    rbm2 = BernoulliRBM(n_visible=args.n_hiddens[i-1],
                        ...
                        momentum=[0.5] * 5 + [0.9],
                        max_epoch=20,#max(args.epochs[1], n_every),
...
parser.add_argument('--epochs', type=int, default=(30, 30, 10), metavar='N', nargs='+',
                    help='number of epochs to train')
parser.add_argument('--batch-size', type=int, default=(480, 480, 1000), metavar='B', nargs='+',
...
# save dirpaths
parser.add_argument('--model_path', type=str, default='', metavar='DIRPATH',
...
def objective(params):
    ...
    print('\n ')
    trials_counter = 0
    args.model_path = 'models_hyperopt/' + script_name + '/trials_' + str(trials_counter) + '/'

    layersAndNodes = list(params['choice'].values()) #len is number of layers
    print("Layers :" + str(len(layersAndNodes)))
    next_layer = 753 # select node between 30 and 784 because 30 is >= display_hidden so subtract 30 * [0,1] then plus 30
    next_layer = min(int(np.ceil(layersAndNodes[0] * next_layer)) + 30,30)
    ...
    for x1 in layersAndNodes[1:]:
        ...
        next_layer = max(next_layer - 24, 24)
        next_layer = max(int(np.ceil(x1 * next_layer)) + 24, 24)
